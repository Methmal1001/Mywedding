...
if (isset($_POST['insert_product'])) {
    ...
    if (empty($product_title) || empty($description) || empty($product_keywords) || empty($product_category) || empty($product_brands)
        ...
        if ($result_query) {
            ...
            // Insert data into orders_pending table with status
            $insert_order_pending = "
                INSERT INTO `orders_pending` 
                (product_id, title, product_image1, date, status) 
                VALUES 
                ('$product_id', '$product_title', '$product_image1', NOW(), 'pending')";

            $result_order_pending = mysqli_query($con, $insert_order_pending);

            if ($result_order_pending) {
                echo "<script>alert('Product and pending order inserted successfully')</script>";
            } else {
    ...
    <div class="container mt-3">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-outline mb-4 w-50 m-auto">
                ...
                <input type="text" name="product_price" id="product_price" class="form-control" placeholder="Enter product price" autocomplete="off" required="not-required">
            </div>
